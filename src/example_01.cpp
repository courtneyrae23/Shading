...
vector<float> cartoonShade(float *normal, bool edge) {
    ...
    float spec_max = pow( max((r[0]*v[0]) + (r[1]*v[1]) + (r[2]*v[2]), (float) 0.0), 200.0 );
...
vector<float> addLight(float *light, float *normal) {
    ...
    float spec_max = pow( max((r[0]*e[0]) + (r[1]*e[1]) + (r[2]*e[2]), (float) 0.0), sp );
...
vector<float> addLightASM(float *light, float *normal) {
    ...
    std::vector<float> Fresnel = {ks[0] + (1.0-ks[0])*pow((1.0 - dot_e_h), 5.0), ks[1] + (1.0-ks[1])*pow((1.0 - dot_e_h), 5.0), ks[2] + (1.0-ks[2])*pow((1.0 - dot_e_h), 5.0)};
    std::vector<float> specular = {Fresnel[0]*s_term1*s_term2, Fresnel[1]*s_term1*s_term2, Fresnel[2]*s_term1*s_term2};

    std::vector<float> sum = {diffuse[0] + specular[0], diffuse[1] + specular[1], diffuse[2] + specular[2]};
